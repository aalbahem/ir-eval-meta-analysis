...
def get_pre_graph_rules():
    ...
    return [S_SR.name, SX_S.name, XS_S.name, P_RN]
...
def validate_metrics(df, metrics=[], runs=[], M=3,print_na=False, print_violation_only=False,metric_prefix=""):
    """
    ...
    rules_names = get_pre_graph_rules()+[Path.name,NA.name]
    for m in metrics:
    ...
    print ("{},{}".format("metric",",".join(rules_names)))
    for m in metrics:
        rules_v = []
        for r in rules_names:
            print_line = (not print_violation_only or metrics_rules_viloations[m][r] > 0)
            rules_v.append(str(metrics_rules_viloations[m][r]))

        print ("{},{}".format(metric_prefix+m, ",".join(rules_v)))
...
if __name__ == '__main__':
    ...
    for m in [10]:
        ...
        metrics = ["avg_ct", "ct", "p"]
        validate_metrics(df, metrics,M=m, print_na=False, print_violation_only=False)
        ...
        validate_metrics(df, metrics=metrics, M=m,print_na=False, print_violation_only=False,metric_prefix="t-")
